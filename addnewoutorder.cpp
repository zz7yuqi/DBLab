...
AddNewoutOrder::AddNewoutOrder(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::AddNewoutOrder)
{
    ui->setupUi(this);
...
AddNewoutOrder::~AddNewoutOrder()
{
    delete ui;
}

void AddNewoutOrder::on_btnNewRow_clicked()
{
...
void AddNewoutOrder::on_btnDelRow_clicked()
{
...
void AddNewoutOrder::on_pushButton_clicked()
{
    //出货单号是否合法
     ...
     //提交出货单至数据库, 缺货登记暂时为false，放在审核员审核时处理

     outorder * newOrder = new outorder(ui->lineEditId->text(), ui->comboBox->currentText(),
                                        uutil::_staff->getId(), 0, ui->textBrowser->toPlainText(), false);

     if (!mysqlutil::submitOutOrder(this,newOrder)){
         //录入失败
         QMessageBox::critical(this, "失败", "出货单录入失败！");
         return;
     }
     ...
     for (int i = 0; i < ui->tableWidget->rowCount(); i++){
         ...
         if (!mysqlutil::submitOutOrderProduct(this,orderId,productId,productNum)){
             //录入失败
             QMessageBox::critical(this, "失败", "出货单产品录入失败！\n失败产品："+productId);
     ...
     //成功
     if (QMessageBox::information(this,"成功","出货单已成功进入数据库，请等待审核！\n") == QMessageBox::Ok){
         //清空
         ui->lineEditId->clear();
